...
(1) Create a workspace named **"ros1_ws"** and a new **"src"** folder within this directory to store the function package. Execute the following commands in the terminal:
...
Save and exit the file. Replace "`~/ros1_ws`" with the actual directory path to your workspace if it's different.
...
:::{Note}
* After making modifications to the program, it's essential to recompile it to ensure that the changes take effect. Use the following command to compile: `sudo colcon build`
